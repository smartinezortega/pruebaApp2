...
// @desc    Delete Generals task
// @route   DELETE /api/tareasgenerales/:id
// @access  Private/Admin

const deleteGeneralTask = asyncHandler(async (req, res) => {
  const { id } = req.params

  const findGeneralTask = `SELECT * FROM tareas WHERE id_tarea = ${id}`
  ...
  try {
    const isExistTask = await queryPromise(findGeneralTask)
...
module.exports = deleteGeneralTask
