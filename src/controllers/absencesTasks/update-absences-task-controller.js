...
// @route   PUT /api/tareasausencias/:id
// @access  Private/Admin
const updateAbsencesTask = asyncHandler(async (req, res) => {
  ...
  try {
        ...
        if (result) {
          db.query(
            ...
            (err, result) => {
              ...
              if (result) {
                res.status(201).json({
                  message: 'Tarea de ausencia editada correctamente',
                })
              }
            }
          )
    ...
    return res.status(400).json({ message: 'Ha ocurrido un error en el guardado de las tareas de ausencia' })
  }
})

module.exports = updateAbsencesTask
