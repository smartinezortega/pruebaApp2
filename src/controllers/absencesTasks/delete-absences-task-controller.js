...
// @route   DELETE /api/tareasausencias/:id
// @access  Private/Admin
const deleteAbsencesTask = asyncHandler(async (req, res) => {
  const { id } = req.params

  const findAbsenceTask = `SELECT * FROM tareas WHERE id_tarea = ${id}`
  ...
  try {
    const isExistTask = await queryPromise(findAbsenceTask)
...
module.exports = deleteAbsencesTask
