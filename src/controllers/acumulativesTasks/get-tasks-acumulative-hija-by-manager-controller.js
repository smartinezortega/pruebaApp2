...
const getAcumulativesTasksHijaByManager = asyncHandler(async (req, res) => {
  ...
  if (isAdminOrSuper) {
    ...
    AND tareas.id_tipo_tarea = (select id_tipo_tarea from tareas where id_tarea = ${id})
    ...
    try {
      ...
      res.status(400).json({ message: 'Error al obtener las tareas acumulativas hija' })
    }
  } else if (isManager) {
...
module.exports = getAcumulativesTasksHijaByManager
