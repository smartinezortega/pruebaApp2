...
// @route   DELETE /api/tareasespecificas/:id
// @access  Private/Admin

const deleteSpecificTask = asyncHandler(async (req, res) => {
  const { id } = req.params

  const findSpecificTask = `SELECT * FROM tareas WHERE id_tarea = ${id}`
  ...
  try {
    const isExistTask = await queryPromise(findSpecificTask)
