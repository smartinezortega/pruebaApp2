...
// @route   PUT /api/tareasespecificas/:id
// @access  Private/Admin
const updateSpecificTask = asyncHandler(async (req, res) => {
  ...
  try {
        ...
        if (result) {
          db.query(
            ...
            (err, result) => {
              ...
              if (result) {
                res.status(201).json({
                  message: 'Tarea específica editada correctamente',
                })
              }
            }
          )
    ...
    return res.status(400).json({ message: 'Ha ocurrido un error en el guardado de las tareas específicas' })
  }
})
